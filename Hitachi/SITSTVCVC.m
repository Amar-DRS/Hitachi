...
- (void)viewDidLoad {
    ...
    ProductVideoInput=  @{@"action":@"product_tvc_video",};
    
    [helper TVCVideoList:ProductVideoInput];
...
-(void)webApiResponseData:(NSData *)responseDictionary apiName:(NSString *)apiName
{
    ...
    if([apiName isEqualToString:@"TVCVideoList"])
    {
        if ([[dataDic valueForKey:@"status"]  isEqual:@1])
        {
            [ProductsArr removeAllObjects];
            ProductsArr=[dataDic valueForKey:@"TVC_Video"];
